...
// Get apps from specific category for homepage
export function getAppsByCategory(category, limit = 6) {
  const db = initDatabase();
  const stmt = db.prepare('SELECT id, data, timestamp FROM app_cache WHERE data LIKE ? ORDER BY timestamp DESC LIMIT ?');
  const categoryPattern = `%"genre":"${category}"%`;
  const results = stmt.all(categoryPattern, limit);
  return results.map(row => ({
    id: row.id,
    timestamp: row.timestamp,
    ...JSON.parse(row.data)
...
// Get apps from search cache by category
export function getAppsByCategoryFromSearch(category, limit = 6) {
  const db = initDatabase();
  const stmt = db.prepare('SELECT results FROM search_cache WHERE query = ? AND timestamp > ?');
  const oneHourAgo = Date.now() - (60 * 60 * 1000);
  const result = stmt.get(category, oneHourAgo);
  ...
  if (result) {
    const apps = JSON.parse(result.results);
